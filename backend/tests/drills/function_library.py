...
def _build_integrand_candidate(rng: random.Random, level: int) -> sp.Expr:
    ...
    # level 9: combined -- either a scaled cyclic-IBP pattern, or a harder
    # u-substitution with a leading coefficient multiplied through
    if rng.random() < 0.5:
        return _nonzero_coeff(rng) * _cyclic_ibp_pattern(rng)
    return _nonzero_coeff(rng) * _u_sub_pattern(rng, inner_max_degree=3)
