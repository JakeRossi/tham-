...
class IntegralsDrill(Drill):
    ...
    def generate(self, difficulty: float, rng_seed: int | None = None) -> Problem:
        ...
        a, b = self._choose_safe_bounds(rng, expr, difficulty)
    ...
    @staticmethod
    def _choose_safe_bounds(rng: random.Random, expr: sp.Expr, difficulty: float, max_attempts: int = 15) -> tuple[int, int]:
        """
        Picks small integer bounds [a, b] that don't straddle a
        singularity of the integrand (e.g. the partial-fractions level's
        1/((x+a)(x+b)) has poles where the denominator hits zero) --
        integrating across a pole gives an undefined/improper result,
        which would make even the drill's own generated answer wrong.
        """
        bound_range = 2 + int(difficulty * 4)

        try:
            denom = sp.denom(sp.together(expr))
            poles = [p for p in sp.solve(denom, x) if p.is_real] if denom != 1 else []
        except Exception:
            poles = []  # if we can't determine poles, fall back to trusting there aren't any
        ...
        for _ in range(max_attempts):
            ...
            if a > b:
                a, b = b, a
            if not any(a <= p <= b for p in poles):
                return a, b

        # Fallback: bounds guaranteed safe -- both below every pole (or
        # [0, 1] if there are somehow no poles found, which never happens
        # for the level that actually has them).
        if poles:
            safe_point = min(poles) - 1
            return int(safe_point) - 1, int(safe_point)
        return 0, 1
