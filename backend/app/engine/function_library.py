...
def _inverse_trig_atom(rng: random.Random, var: sp.Expr) -> sp.Expr:
    func = rng.choice([sp.asin, sp.acos, sp.atan])
    a = rng.randint(1, 3)
    return func(a * var)


def _log_atom(rng: random.Random, var: sp.Expr) -> sp.Expr:
    a = rng.randint(1, 3)
    return sp.log(a * var)
...
def generate_single_var_expr(rng: random.Random, level: int) -> sp.Expr:
    ...
    if level == 9:
        inner = _poly_term_nonconstant(rng, x, 2)
        outer = rng.choice([sp.sin, sp.cos, sp.exp, sp.log, sp.asin, sp.atan])
        return outer(inner)
    # level 10: combined -- product/quotient/chain, OR genuine DOUBLE
    # composition (nested chain rule, e.g. sin(cos(x^2)) or ln(sin(x)+2)),
    # which needs the chain rule applied TWICE and is meaningfully harder
    # than level 9's single composition.
    combo = rng.choice(["product_chain", "quotient_chain", "triple_product", "double_chain"])
    if combo == "triple_product":
        return _simple_atom(rng, x) * _simple_atom(rng, x) * _poly_term(rng, x, 1)
    if combo == "double_chain":
        innermost = _poly_term_nonconstant(rng, x, 2)
        middle_func = rng.choice([sp.sin, sp.cos])
        middle = middle_func(innermost)
        outer_func = rng.choice([sp.sin, sp.cos, sp.exp, sp.log])
        # log needs a positive-safe argument in form (won't matter for symbolic
        # differentiation, but add a constant so it's not just log(sin(...)) verbatim)
        return outer_func(middle + rng.randint(2, 4)) if outer_func is sp.log else outer_func(middle)
    inner = _poly_term_nonconstant(rng, x, 2)
